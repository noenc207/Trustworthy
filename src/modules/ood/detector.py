...
from src.modules.ood.interfaces import OODStrategy, OODDetector
...
from src.modules.ood.strategies import MSPStrategy, EnergyStrategy, EntropyStrategy, MahalanobisStrategy
        ...
        try:
            ...
            if self.config.fail_safe_conservative:
                ...
                return OODResult(
                    ...
                    algorithm=self.strategy.metadata()["name"],
                    threshold=self.config.threshold,
                    reason=f"OOD evaluation failed: {str(e)}. Conservative fallback applied.",
